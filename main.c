...
#include <math.h>
#include <time.h>
//İkinci ogretimlerin birinci ogretimlerden sonraki kayit siralarini bulmak icin bu fonksiyonu kullanıyoruz
...
//Ayni isimden öğrenci varsa fazla olanlarin bilgilerine X atiyoruz
...
void kayit_guncelleme(char isim[1000][15],char soyisim[1000][15],char kayit_sirasi[1000][5],char ogretim_turu[1000][5],char numara[1000][10],int boyut,int ayni[4])
{
    ...
    for(i=0; i<boyut; i++)//Ayni kayittan olduğu için silinen yani ismine X atilan kayit kaydırılarak üstüne siradaki kayit yerleşiyor
    {
...
//Dolu birinci ve ikinci ogretim siniflarina göre sınıf dosyası oluşturuyoruz
...
int main()
{
    ...
    int kontrol1[boyut];
    //Kac tane birinci ve ikinci oğretim oğrencisi var onu buluyoruz
    for(i=0; i<boyut-ayni[1]-ayni[2]; i++)
    {
